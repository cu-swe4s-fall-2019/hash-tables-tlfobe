...
class TestLinearProbe(unittest.TestCase):
    def test_linear_probe_incorrect_types(self):
        self.assertRaises(TypeError, hash_tables.LinearProbe, hash_functions.h_ascii, "not an int!")
        self.assertRaises(NotImplementedError, hash_tables.LinearProbe, lambda a,b: None, 50)
        
    def test_linear_probe_add_empty(self):
        table = hash_tables.LinearProbe(hash_functions.h_ascii, 100)
        assert(table.add('woah!', 1) == True)
    ...
    def test_linear_probe_add_search_random(self):
        ...
        assert(table.add(key, value) == True)
