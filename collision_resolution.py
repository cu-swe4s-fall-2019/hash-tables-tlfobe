...
def main():
    parser = argparse.ArgumentParser(description="A program to plot " +
                                     "collision strategies of various hash " +
                                     "functions hash tables",
                                     )
    ...
    try:
        ...
        print("collision_resolution: file selected was not a datafile",
              file=sys.stderr)
        sys.exit(1)
    except FileNotFoundError:
        print("collision_resolution: please input a valid file path",
              file=sys.stderr)
        sys.exit(1)
    ...
    if args.col_res == "QuadraticProbe":
        # Quadratic Probing is not guarenteed to fill the entire array
        N = int(2*N+1)
    ...
    plt.savefig(args.out_id+"_"+args.col_res+"_" +
                args.hash_fxn+"_add.png", bbox_inches='tight')
    ...
    ax.plot(list(range(1, len(times_to_search)+1)),
            times_to_search, '.', ms=1, alpha=0.5)
    ax.set_xlabel("Hashed Word")
    ax.set_ylabel("Search Time")

    plt.savefig(args.out_id+"_"+args.col_res+"_" +
                args.hash_fxn+"_search.png", bbox_inches='tight')
